...
def _fixture_internal_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["mkdocs_spellcheck._internal"]


@pytest.fixture(name="public_api", scope="module")
def _fixture_public_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["mkdocs_spellcheck"]
...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `mkdocs_spellcheck`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in mkdocs_spellcheck.__all__ or not hasattr(mkdocs_spellcheck, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in mkdocs_spellcheck.__all__:
...
def test_inventory_matches_api(
    ...
    for item in inventory.values():
        ...
        if (
            ...
            and (item.name == "mkdocs_spellcheck" or item.name.startswith("mkdocs_spellcheck."))
        ):
